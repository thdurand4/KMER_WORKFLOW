...
# 1005    Erianthus_fulvus_EF001-JGI      Narenga_porphyrocoma_Narenga-JGI        Narenga_sp_N001-JGI
# 864     Miscanthus_Floridus_MiscFlo-PI295762-JGI        Miscanthus_sinense_JW484-JGI
# 814     Miscanthus_Floridus_MiscFlo-PI295762-JGI        Miscanthus_sinense_JW484-JGI    Miscanthus_sinense_NG7722-JGI   Saccharum_x_Miscanthus_hybrid_Toaeho-JGI
# 804     Miscanthus_sinense_JW484-JGI    Miscanthus_sinense_NG7722-JGI
# 733     Erianthus_arundinaceus_EA001-JGI        Erianthus_arundinaceus_IK_76-48-JGI
...
my $marge_haute=10*$police; 								# marge axe X 
...
while (my $line =<IN>){
	...
	if ($line =~ /^$/){
		...
		splice @LIST_ACCESSION_IN_FILE, 0, 1;
...
while (my $line =<IN>){
	...
	$nb_accession_in_current_intersection=(scalar(@table_intersection))-1;
	$first_accession_in_intersection=$table_intersection[1];
	...
	foreach my $accession (@table_intersection){	
		...
		$img->char(gdLargeFont,$coordonnee_X,$coordonnee_Y,$accession,$color_svg); #$img->stringFT($color_svg,'/home/garsmeuro/Fonts/TIMES.TTF',36,-90,$ACC_X,$ACC_Y,$accession);	
		...
		if ($nb_accession_in_current_intersection <2){
			...
			#print $coordonnee_Histo_specific_X+($nb_kmer_in_intersection/(5000*$police)),"\n";
		}
	}
}
...
while (my $line =<IN>){
	...
	if ($nb_accession_in_current_intersection >=2){
		...
		foreach my $accession (@table_intersection){
			...
			$img->stringUp(gdLargeFont,$INTERSECTION_X-10,$INTERSECTION_Y+$marge_haute-20,$nb_kmer_in_intersection,$black_txt_color);
